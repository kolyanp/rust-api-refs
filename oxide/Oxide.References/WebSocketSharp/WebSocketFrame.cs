...
internal class WebSocketFrame : IEnumerable<byte>, IEnumerable
{
	...
	private static string dump(WebSocketFrame frame)
	{
		...
		Func<Action<string, string, string, string>> func = delegate
		{
			...
			{
				output.AppendFormat(lineFmt, new object[5]
				{
					++lineCnt,
					text,
					text2,
	...
	private static string print(WebSocketFrame frame)
	{
		...
		return string.Format(format, new object[10] { frame._fin, frame._rsv1, frame._rsv2, frame._rsv3, frame._opcode, frame._mask, payloadLength, text, text2, text3 });
	}
