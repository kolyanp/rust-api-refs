...
public sealed class ProtoReader : IDisposable
{
	...
	internal static Exception AddErrorData(Exception exception, ProtoReader source)
	{
		...
		{
			exception.Data.Add("protoSource", string.Format("tag={0}; wire-type={1}; offset={2}; depth={3}", new object[4] { source.fieldNumber, source.wireType, source.position, source.depth }));
		}
		return exception;
	}
