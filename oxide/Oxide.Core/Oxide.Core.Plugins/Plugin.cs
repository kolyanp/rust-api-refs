...
public abstract class Plugin
{
		...
		finally
		{
			...
			if (num > 0f)
			{
				...
				if (totalSeconds > 0.1)
				{
					...
					Interface.Oxide.LogWarning(string.Format("Calling '{0}' on '{1} v{2}' took {3:0}ms{4}", new object[5]
					{
						hook,
						Name,
						Version,
				...
				if (num3 - (double)averageAt > 10.0)
				{
					...
					if (num2 > 0.1)
					{
						...
						Interface.Oxide.LogWarning(string.Format("Calling '{0}' on '{1} v{2}' took average {3:0}ms{4}", new object[5]
						{
							hook,
							Name,
							Version,
	...
	{
		AddCovalenceCommand(commands, perms, delegate(IPlayer caller, string text, string[] args)
		{
			CallHook(callback, caller, text, args);
			return true;
		});
		Covalence library = Interface.Oxide.GetLibrary<Covalence>();
		foreach (string command in commands)
		{
			library.RegisterCommand(command, this, CovalenceCommandCallback);
		}
	}
