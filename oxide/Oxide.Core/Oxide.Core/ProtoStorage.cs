...
public class ProtoStorage
{
	...
	public static T Load<T>(params string[] subPaths)
	{
		...
		try
		{
			...
			{
				using (FileStream source = File.OpenRead(fileDataPath))
				{
	...
	public static void Save<T>(T data, params string[] subPaths)
	{
		...
		try
		{
			...
			using FileStream destination = File.Open(fileDataPath, mode);
			Serializer.Serialize((Stream)destination, data);
		}
		catch (Exception ex)
		{
