	...
	private static bool DownloadFile(string url, string path, int retries = 3)
	{
		...
		try
		{
			...
			if (!TryDownload(url, retries, ref current, lastModified, out var data, out var code, out var newerFound, ref md))
			{
				...
				Interface.Oxide.LogError(string.Format("[CSharp] Failed to download {0} after {1} {2} with response code '{3}', please manually download it from {4} and save it here {5}", new object[6] { fileName, current, text2, code, url, path }));
				return false;
			}
			if (data != null)
			{
