...
public class RustCommandSystem : ICommandSystem
{
	...
	public void RegisterCommand(string command, Plugin plugin, CommandCallback callback)
	{
		...
		if (cmdlib.consoleCommands.TryGetValue(text3, out var value3))
		{
			...
			ConsoleSystem.Index.Server.Dict.Remove(StringView.op_Implicit(value3.RustCommand.FullName));
			if (text == "global")
			{
				ConsoleSystem.Index.Server.GlobalDict.Remove(StringView.op_Implicit(value3.RustCommand.Name));
			}
			ConsoleSystem.Index.All = ConsoleSystem.Index.Server.Dict.Values.ToArray();
			cmdlib.consoleCommands.Remove(value3.RustCommand.FullName);
		}
		if (ConsoleSystem.Index.Server.Dict.TryGetValue(StringView.op_Implicit(text3), out var value4))
		{
			if (value4.Variable)
			{
		...
		registeredCommand.RustCommand = new ConsoleSystem.Command
		{
			...
			Call = delegate(ConsoleSystem.Arg arg)
			{
				if (arg != null)
				{
					...
					if (arg.Connection != null && (Object)(object)basePlayer != (Object)null)
					{
		...
		ConsoleSystem.Index.Server.Dict[StringView.op_Implicit(text3)] = registeredCommand.RustCommand;
		if (text == "global")
		{
			ConsoleSystem.Index.Server.GlobalDict[StringView.op_Implicit(text2)] = registeredCommand.RustCommand;
		}
		ConsoleSystem.Index.All = ConsoleSystem.Index.Server.Dict.Values.ToArray();
