...
public class Command : Library
{
	...
	public void AddConsoleCommand(string command, Plugin plugin, Func<ConsoleSystem.Arg, bool> callback)
	{
		...
		if (consoleCommands.TryGetValue(text3, out var value))
		{
			...
			Interface.Oxide.LogWarning(format);
			ConsoleSystem.Index.Server.Dict.Remove(StringView.op_Implicit(value.RustCommand.FullName));
			if (text == "global")
			{
				ConsoleSystem.Index.Server.GlobalDict.Remove(StringView.op_Implicit(value.RustCommand.Name));
			}
		...
		if (ConsoleSystem.Index.Server.Dict.TryGetValue(StringView.op_Implicit(text3), out var value3))
		{
			if (value3.Variable)
			{
		...
		ConsoleSystem.Index.Server.Dict[StringView.op_Implicit(text3)] = consoleCommand.RustCommand;
		if (text == "global")
		{
			ConsoleSystem.Index.Server.GlobalDict[StringView.op_Implicit(text2)] = consoleCommand.RustCommand;
		}
		ConsoleSystem.Index.All = ConsoleSystem.Index.Server.Dict.Values.ToArray();
	...
	private void RemoveConsoleCommand(ConsoleCommand command)
	{
		...
		{
			ConsoleSystem.Index.Server.Dict[StringView.op_Implicit(command.RustCommand.FullName)].Call = command.OriginalCallback;
			if (command.RustCommand.FullName.StartsWith("global."))
			{
				ConsoleSystem.Index.Server.GlobalDict[StringView.op_Implicit(command.RustCommand.Name)].Call = command.OriginalCallback;
			}
			return;
		}
		ConsoleSystem.Index.Server.Dict.Remove(StringView.op_Implicit(command.RustCommand.FullName));
		if (command.Name.StartsWith("global."))
		{
			ConsoleSystem.Index.Server.GlobalDict.Remove(StringView.op_Implicit(command.RustCommand.Name));
		}
		ConsoleSystem.Index.All = ConsoleSystem.Index.Server.Dict.Values.ToArray();
	}
