...
public class PatrolHelicopterAI : BaseMonoBehaviour
{
	...
	public void UpdateTargetList()
	{
		...
		{
			targetinfo targetinfo3 = _targetList[num2];
			if (targetinfo3 == null || !targetinfo3.ent.IsValid())
			{
				RemoveTargetAt(num2);
			}
			else if (use_danger_zones && IsInNoGoZone(((Component)targetinfo3.ply).transform.position))
			{
				RemoveTargetAt(num2);
			}
			else if (AI.ignoreplayers || SimpleAIMemory.PlayerIgnoreList.Contains(targetinfo3.ply))
			{
				RemoveTargetAt(num2);
			}
			else if (DeepSeaManager.IsInsideDeepSea((BaseNetworkable)targetinfo3.ply))
			{
				RemoveTargetAt(num2);
			}
			else
			{
				...
				if (targetinfo3.TimeSinceSeen() >= 6f || flag2)
				{
					...
					if ((CanStrafe() || CanUseNapalm()) && IsAlive() && !flag && !flag2 && ((Object)(object)targetinfo3.ply == (Object)(object)leftGun._target || (Object)(object)targetinfo3.ply == (Object)(object)rightGun._target) && flag3)
					{
						...
						flag = true;
						strafeTarget = targetinfo3.ply;
					}
					RemoveTargetAt(num2);
					if ((Object)(object)leftGun._target == (Object)(object)targetinfo3.ply)
					{
						leftGun._target = null;
					}
					if ((Object)(object)rightGun._target == (Object)(object)targetinfo3.ply)
					{
						rightGun._target = null;
					}
				}
				if (use_danger_zones && !flag && (CanStrafe() || CanUseNapalm()) && IsAlive() && (Time.realtimeSinceStartup - lastNapalmTime > 20f || Time.realtimeSinceStartup - lastStrafeTime > 15f) && IsInDangerZone(((Component)targetinfo3.ply).transform.position, out var dangerZone) && dangerZone != null && dangerZone.Score > num)
				{
		...
		if (use_danger_zones && !flag && targetinfo2 != null)
		{
	...
	private void AddNewTargetsToList()
	{
		...
		using (TimeWarning.New("PatrolHelicoperAI.AddNewTargetsToList"))
		{
			...
			try
			{
				BaseEntity.Query.Server.GetPlayersInSphere(((Component)this).transform.position, 150f, (List<BasePlayer>)(object)val, BaseEntity.Query.DistanceCheckType.None);
				foreach (BasePlayer item in (List<BasePlayer>)(object)val)
				{
					...
					if (item.IsInTutorial || DeepSeaManager.IsInsideDeepSea((BaseNetworkable)item))
					{
						continue;
					}
	...
	private static MonumentInfo GetCloseMonument(float maxDistance, Vector3 position)
	{
		...
		if ((Object)(object)TerrainMeta.Path != (Object)null && TerrainMeta.Path.Monuments != null && TerrainMeta.Path.Monuments.Count > 0)
		{
			...
			foreach (MonumentInfo monument in TerrainMeta.Path.Monuments)
			{
				...
				if (type != MonumentType.Mountain && type != MonumentType.Lighthouse && type != MonumentType.Lake && type != MonumentType.WaterWell && type != MonumentType.Cave && type != MonumentType.Building && monument.Tier != 0 && !(((Component)monument).transform.position.y < WaterSystem.OceanLevel) && monument.AllowPatrolHeliCrash)
				{
	...
	private void StartStrafe(BasePlayer strafeTarget, bool shouldUseNapalm = false)
	{
		...
		if (Interface.CallHook("OnHelicopterStrafeEnter", this, ((Component)strafeTarget).transform.position, strafeTarget) == null)
		{
