...
public class HexagonGridGenerator : BaseEntity
{
	...
	{
		int value = args.GetInt(0, 5);
		float num = args.GetFloat(1, 1.35f);
		value = Math.Clamp(value, 1, 50);
		num = Mathf.Clamp(num, 1.35f, float.PositiveInfinity);
		Instance.gridRadius = value;
		Instance.tileSpacing = num;
