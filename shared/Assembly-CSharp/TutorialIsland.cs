...
public class TutorialIsland : BaseEntity, IEntityPingSource
{
	...
	private static TutorialIsland CreateIslandForPlayer(BasePlayer player)
	{
		...
		Group obj = SetupGroup(unusedTutorialIslandLocationRotation, worldPos, worldRot);
		...
		AddIslandBounds(tutorialIsland.WorldSpaceBounds(), obj.ID, isServer: true);
	...
	private static void AddIslandBounds(OBB worldBounds, uint netId, bool isServer)
	{
		...
		IslandBounds islandBounds = new IslandBounds
		{
			Id = netId,
		...
		if (isServer)
		{
			BoundsListServer.TryAdd(islandBounds);
		}
	}
