...
public class OcclusionCulling : MonoBehaviour
{
	...
	public static bool Enabled
	{
		...
		set
		{
			...
			{
				((Behaviour)instance).enabled = enabled;
			}
		}
	}
	...
	private void OnEnable()
	{
		...
		if (!Supported)
		{
			Debug.LogWarning((object)("[OcclusionCulling] Disabled due to graphics device type " + ((object)SystemInfo.graphicsDeviceType/*cast due to constrained. prefix*/).ToString() + " not supported."));
			Enabled = false;
			return;
		}
		...
		{
			Debug.LogWarning((object)("[OcclusionCulling] Disabled due to unsupported Async GPU Reads on device " + ((object)SystemInfo.graphicsDeviceType/*cast due to constrained. prefix*/).ToString()));
			Enabled = false;
			return;
		}
