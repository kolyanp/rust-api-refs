...
public class PostProcessLayer : MonoBehaviour
{
	...
	private void BuildCommandBuffers()
	{
		...
		if (num > 0)
		{
			...
			{
				RuntimeUtilities.BuiltinBlit(val4, currentContext.source, currentContext.destination, RuntimeUtilities.copyStdMaterial, stopNaNPropagation ? 1 : 0);
	...
	private void BuildPostEffectsOld(RenderTextureFormat sourceFormat, PostProcessRenderContext context, RenderTargetIdentifier cameraTarget)
	{
		...
		if (RequiresInitialBlit(m_Camera, context) | flag)
		{
			...
			RuntimeUtilities.BuiltinBlit(m_LegacyCmdBuffer, cameraTarget, RenderTargetIdentifier.op_Implicit(num), RuntimeUtilities.copyStdMaterial, stopNaNPropagation ? 1 : 0);
			if (!m_NaNKilled)
			{
		...
		for (int i = 0; i < context.numberOfEyes; i++)
		{
			...
			if (stopNaNPropagation && !m_NaNKilled)
			{
				...
				if (context.stereoActive && context.numberOfEyes > 1)
				{
					...
					{
						RuntimeUtilities.BlitFullscreenTriangleFromDoubleWide(command, context.source, RenderTargetIdentifier.op_Implicit(num), RuntimeUtilities.copyStdFromDoubleWideMaterial, 1, i);
						flag = true;
					}
				}
				else
			...
			if (!flag && context.numberOfEyes > 1)
			{
				...
				if (context.stereoActive)
				{
					...
					{
						RuntimeUtilities.BlitFullscreenTriangleFromDoubleWide(command, context.source, RenderTargetIdentifier.op_Implicit(num), RuntimeUtilities.copyStdFromDoubleWideMaterial, stopNaNPropagation ? 1 : 0, i);
						flag = true;
					}
				}
				context.source = RenderTargetIdentifier.op_Implicit(num);
	...
	private int RenderEffect<T>(PostProcessRenderContext context, bool useTempTarget = false) where T : PostProcessEffectSettings
	{
		...
		_ = Graphics.detailed_postprocessing_profiling;
