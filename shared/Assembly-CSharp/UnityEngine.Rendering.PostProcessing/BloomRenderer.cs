...
internal sealed class BloomRenderer : PostProcessEffectRenderer<Bloom>
{
	...
	public override void Render(PostProcessRenderContext context)
	{
		...
		propertySheet.properties.SetVector(UnityEngine.Rendering.PostProcessing.ShaderIDs.Threshold, val);
		float num13 = Mathf.GammaToLinearSpace(base.settings.clamp.value);
		propertySheet.properties.SetVector(UnityEngine.Rendering.PostProcessing.ShaderIDs.Params, new Vector4(num13, 0f, 0f, 0f));
		...
		for (int num16 = num9 - 2; num16 >= 0; num16--)
		{
			...
			command.SetGlobalTexture(UnityEngine.Rendering.PostProcessing.ShaderIDs.BloomTex, RenderTargetIdentifier.op_Implicit(down2));
		...
		{
			propertySheet.properties.SetVector(UnityEngine.Rendering.PostProcessing.ShaderIDs.ColorIntensity, new Vector4(linear.r, linear.g, linear.b, num17));
		...
		uberSheet.properties.SetVector(UnityEngine.Rendering.PostProcessing.ShaderIDs.Bloom_DirtTileOffset, val4);
		uberSheet.properties.SetVector(UnityEngine.Rendering.PostProcessing.ShaderIDs.Bloom_Settings, val2);
		uberSheet.properties.SetColor(UnityEngine.Rendering.PostProcessing.ShaderIDs.Bloom_Color, linear);
		uberSheet.properties.SetTexture(UnityEngine.Rendering.PostProcessing.ShaderIDs.Bloom_DirtTex, val3);
		command.SetGlobalTexture(UnityEngine.Rendering.PostProcessing.ShaderIDs.BloomTex, RenderTargetIdentifier.op_Implicit(num15));
