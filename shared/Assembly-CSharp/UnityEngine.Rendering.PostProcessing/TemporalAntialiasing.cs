...
public sealed class TemporalAntialiasing
{
	...
	private RenderTexture CheckHistory(int id, PostProcessRenderContext context)
	{
		...
		if (m_ResetHistory || (Object)(object)val == (Object)null || !val.IsCreated())
		{
			...
			RuntimeUtilities.BlitFullscreenTriangle(context.command, context.source, RenderTargetIdentifier.op_Implicit((Texture)(object)val));
		}
		else if (((Texture)val).width != context.width || ((Texture)val).height != context.height)
		{
			...
			RuntimeUtilities.BlitFullscreenTriangle(context.command, RenderTargetIdentifier.op_Implicit((Texture)(object)val), RenderTargetIdentifier.op_Implicit((Texture)(object)screenSpaceTemporaryRT));
			RenderTexture.ReleaseTemporary(val);
		}
