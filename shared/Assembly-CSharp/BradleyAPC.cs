...
public class BradleyAPC : BaseCombatEntity, TriggerHurtNotChild.IHurtTriggerUser, IPathListener
{
	...
	private ScientistNPC SpawnScientist(GameObjectRef scientistPrefab, Vector3 spawnPos, bool roadSpawned)
	{
		...
		RustNavMeshAgent component = ((Component)scientistNPC).GetComponent<RustNavMeshAgent>();
	...
	public void DoSimpleAI()
	{
		...
		if (!IsOnSpline())
		{
			...
			if (num > stoppingDist)
			{
				...
				float num6 = Vector3.Dot(myRigidBody.linearVelocity, ((Component)this).transform.forward);
	...
	public override void OnDied(HitInfo info)
	{
		...
		for (int j = 0; j < maxCratesToSpawn; j++)
		{
			...
			val.linearVelocity = zero + onUnitSphere2 * Random.Range(1f, 3f);
			val.angularVelocity = Vector3Ex.Range(-1.75f, 1.75f);
			val.linearDamping = 0.5f * (val.mass / 5f);
			val.angularDamping = 0.2f * (val.mass / 5f);
	...
	public float GetDamageMultiplier(BaseEntity ent)
	{
		...
		float num2 = Vector3.Dot(myRigidBody.linearVelocity, ((Component)this).transform.forward);
		if (num2 > 0f)
		{
			num += num2 * 0.5f;
		}
