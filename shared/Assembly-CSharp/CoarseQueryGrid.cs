...
public struct CoarseQueryGrid : IDisposable
{
	public struct RefCountedSpatialArray : IDisposable
	{
		...
		public static RefCountedSpatialArray Init(int length)
		{
			...
			return new RefCountedSpatialArray
			{
				_array = new NativeArray<byte>(length, (Allocator)4, (NativeArrayOptions)1)
			};
		}
	...
	public bool CheckJob(Bounds checkBounds)
	{
		...
		CheckBoundsJob checkBoundsJob = new CheckBoundsJob
		{
			Grid = this,
			CheckBounds = checkBounds,
			Result = _result
		};
		IJobExtensions.RunByRef<CheckBoundsJob>(ref checkBoundsJob);
	...
	public bool CheckJob(Vector3 worldPosition, float radius)
	{
		...
		CheckPosRadJob checkPosRadJob = new CheckPosRadJob
		{
			Grid = this,
			CheckPos = worldPosition,
			CheckRad = radius,
			Result = _result
		};
		IJobExtensions.RunByRef<CheckPosRadJob>(ref checkPosRadJob);
		return _result.Value;
	}
	...
	public JobHandle CheckJob(ReadOnly<Vector3> worldPositions, ReadOnly<float> radii, NativeList<int> overlapIndices)
	{
		...
		CheckPosRadBatchJob checkPosRadBatchJob = new CheckPosRadBatchJob
		{
			Grid = this,
			Pos = worldPositions,
			Radii = radii,
		...
		return IJobExtensions.ScheduleByRef<CheckPosRadBatchJob>(ref checkPosRadBatchJob, default(JobHandle));
	}
	...
	public JobHandle CheckJobIndirect(ReadOnly<Vector3> worldPositions, ReadOnly<float> radii, ReadOnly<int> indices, NativeList<int> overlapIndices)
	{
		...
		CheckPosRadBatchJobIndirect checkPosRadBatchJobIndirect = new CheckPosRadBatchJobIndirect
		{
			Grid = this,
			Pos = worldPositions,
			Radii = radii,
		...
		return IJobExtensions.ScheduleByRef<CheckPosRadBatchJobIndirect>(ref checkPosRadBatchJobIndirect, default(JobHandle));
	}
	...
	public JobHandle CheckJobIndirect(ReadOnly<Vector3> starts, ReadOnly<Vector3> ends, ReadOnly<float> radii, ReadOnly<int> indices, NativeList<int> overlapIndices)
	{
		...
		CheckBoundsJobIndirect checkBoundsJobIndirect = new CheckBoundsJobIndirect
		{
			Grid = this,
			Starts = starts,
			Ends = ends,
		...
		return IJobExtensions.ScheduleByRef<CheckBoundsJobIndirect>(ref checkBoundsJobIndirect, default(JobHandle));
	}
