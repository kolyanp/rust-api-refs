...
public class CH47FlightTest : MonoBehaviour
{
	...
	public HelicopterInputState_t GetHelicopterInputState()
	{
		...
		HelicopterInputState_t result = new HelicopterInputState_t
		{
			throttle = (((ButtonControl)Keyboard.current[(Key)37]).isPressed ? 1f : 0f)
		};
		result.throttle -= (((ButtonControl)Keyboard.current[(Key)33]).isPressed ? 1f : 0f);
		Vector2 val = MouseDelta.Read();
		result.pitch = val.y;
		result.roll = 0f - val.x;
		result.yaw = (((ButtonControl)Keyboard.current[(Key)18]).isPressed ? 1f : 0f);
		result.yaw -= (((ButtonControl)Keyboard.current[(Key)15]).isPressed ? 1f : 0f);
