...
public class GenerateRailRing : ProceduralComponent
{
	...
	public override void Process(uint seed)
	{
		...
		{
			RingNode ringNode = list[num6];
			RingNode next = list[(num6 + 1) % list.Count];
			RingNode prev = list[(num6 - 1 + list.Count) % list.Count];
			ringNode.next = next;
			ringNode.prev = prev;
		...
		{
			RingNode ringNode3 = list[num9];
			RingNode ringNode4 = list[(num9 + 1) % list.Count];
			...
			for (PathFinder.Node node3 = ringNode3.path; node3 != null; node3 = node3.next)
			{
				...
				{
					int num10 = Mathf.Abs(node3.point.x - node4.point.x);
					int num11 = Mathf.Abs(node3.point.y - node4.point.y);
					if (num10 <= 15 && num11 <= 15)
					{
		...
		{
			RingNode ringNode5 = list[num12];
			RingNode ringNode6 = list[(num12 + 1) % list.Count];
			...
			for (PathFinder.Node node8 = ringNode5.path; node8 != null; node8 = node8.next)
			{
				...
				{
					int num13 = Mathf.Abs(node8.point.x - node9.point.x);
					int num14 = Mathf.Abs(node8.point.y - node9.point.y);
					if (num13 <= 1 && num14 <= 1)
					{
		...
		for (PathFinder.Node node12 = node10; node12 != null; node12 = node12.next)
		{
			...
			list2.Add(new Vector3(num15, num17, num16));
		}
		if (list2.Count >= 2)
		{
