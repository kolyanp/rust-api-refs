...
public class IOEntity : DecayEntity
{
	...
	public static bool infiniteIoPower
	{
		...
		set
		{
			...
			try
			{
				...
				{
					if (!(enumerator.Current is IOEntity iOEntity) || (iOEntity.GetQueueType() != QueueType.ElectricLowPriority && iOEntity.GetQueueType() != QueueType.Industrial))
					{
						continue;
					}
					if (infiniteIoPower)
	...
	public virtual void Init()
	{
		...
		if (IsRootEntity())
		{
			Invoke(MarkDirtyForceUpdateOutputs, Random.Range(1f, 1f));
		}
		ApplyInfinitePower();
	}
	...
	{
		if (!infiniteIoPower || (GetQueueType() != QueueType.ElectricLowPriority && GetQueueType() != QueueType.Industrial))
		{
			return;
		}
		for (int i = 0; i < inputs.Length; i++)
	...
	public void FindContainerSource(List<ContainerInputOutput> found, int depth, bool input, List<IOEntity> ignoreList, int parentId = -1, int stackSize = 0)
	{
		...
		foreach (IOSlot iOSlot in array)
		{
			...
			int num3 = -1;
			if (iOEntity is IIndustrialStorage storage)
			{
				num = iOSlot.connectedToSlot;
				if (GetExistingCount(storage) < 2)
				{
		...
		int GetExistingCount(IIndustrialStorage industrialStorage)
		{
			int num4 = 0;
			foreach (ContainerInputOutput item2 in found)
			{
				if (item2.Storage == industrialStorage)
				{
					num4++;
				}
			}
	...
	public override void Reskin_Preserve(ref SprayCan.ReskinPreserveInfo preserveInfo)
	{
		...
		ioEntityPreserve.connectionInfos = Pool.Get<List<ConnectionPreserveInfo>>();
	...
	public override void Reskin_Restore(ref SprayCan.ReskinPreserveInfo preserveInfo)
	{
		...
		{
			if (!((Object)(object)ioEntityPreserve.inputInfos[j].connectedTo == (Object)null))
			{
		...
		foreach (ConnectionPreserveInfo connectionInfo in ioEntityPreserve.connectionInfos)
		{
			...
			if (!((Object)(object)connectionInfo.connectedEntity == (Object)null))
			{
				...
				hashSet.Add(connectionInfo.connectedEntity);
			}
		}
		SnapLinesToHandlePositions(hashSet);
		...
		Pool.FreeUnmanaged<ConnectionPreserveInfo>(ref ioEntityPreserve.connectionInfos);
	}
	...
	public void SnapLinesToHandlePositions(HashSet<IOEntity> toUpdate)
	{
		...
		foreach (IOSlot iOSlot2 in array)
		{
			if (iOSlot2.IsConnected())
			{
				...
				Vector3 val = ((Component)this).transform.TransformPoint(iOSlot2.handlePosition);
				obj.linePoints[0] = ((Component)iOEntity).transform.InverseTransformPoint(val);
