...
public class BaseNavigator : BaseMonoBehaviour
{
	...
	public void SetNavMeshEnabled(bool flag)
	{
		...
		if (((Behaviour)Agent).enabled == flag)
		{
			return;
		}
	...
	public bool PlaceOnNavMesh(float yOffset)
	{
		...
		if (GetNearestNavmeshPosition(((Component)this).transform.position + Vector3.up * yOffset, out var position, maxRange))
		{
			flag = Warp(position);
			if (flag)
			{
	...
	private void UpdateMovement(float delta)
	{
		...
		{
			stuckTimer += delta;
			if (CurrentNavigationType != NavigationType.None && stuckTimer >= stuckTriggerDuration)
			{
