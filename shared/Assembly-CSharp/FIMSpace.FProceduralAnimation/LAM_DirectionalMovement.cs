...
public class LAM_DirectionalMovement : LegsAnimatorControlModuleBase
{
	...
	private class LegRedirectHelper
	{
		...
		public Vector3 ComputeIKOffset(Vector3 localPos, float smoother = 1f)
		{
			...
			float num = parent._play_TrDur.GetFloat();
			Vector3 val = parent._calc_LocalRotDir * localPos;
			float num2 = Vector3.Magnitude(val - computedPosLocal);
			float num3 = num2 / leg.Owner.ScaleReferenceNoScale;
			float num4 = 0f;
			if (num2 > 0.2f)
			{
				num4 = Mathf.InverseLerp(0.2f, 1f, num2);
				num4 *= 0.1f;
				if (num < 0.1f)
				{
					num4 *= num / 0.1f;
			...
			if (!(smoother >= 3f))
			{
				if (smoother > 0f)
				{
					if (num3 < 0.1f / smoother)
					{
						computedPosLocal = val;
					}
					else
					{
						num5 = Mathf.InverseLerp(1.5f * smoother, 0.1f / smoother, num3) * 6f;
					}
			...
			computedPosLocal = Vector3.Lerp(computedPosLocal, val, LA.DeltaTime * (Mathf.Lerp(20f, 4f, (parent._calc_angleDiffFactor * num + num4) * 1.5f) + num5));
			...
			if (parent._var_raiseLimit > 0f)
			{
				...
				{
					num6 = Mathf.Lerp(0f, 0.5f, parent._calc_sideFactorR);
				}
				else if (leg.Side == LegsAnimator.ELegSide.Right)
				{
					num6 = Mathf.Lerp(0f, 0.5f, parent._calc_sideFactorL);
				}
				if (parent._calc_toNegativeXProgress > 0f)
				{
					num6 = Mathf.Lerp(num6, 1f, parent._calc_toNegativeXProgress);
				}
				computedPosLocal.y = Mathf.Lerp(computedPosLocal.y, c_AnimatedAnkleFlatHeight, num6 * parent._var_raiseLimit);
			}
	...
	public override void OnPreLateUpdate(LegsAnimator.LegsAnimatorCustomModuleHelper helper)
	{
		...
		float num3 = ((!(_wrappedAngle < 90f)) ? Mathf.InverseLerp(180f, 90f, wrappedAngle) : Mathf.InverseLerp(0f, 90f, wrappedAngle));
		_calc_sideFactorR = Mathf.Lerp(_calc_sideFactorR, num3, _calc_deltaSpeed * 2f);
		num3 = ((!(_wrappedAngle > -90f)) ? Mathf.InverseLerp(-180f, -90f, wrappedAngle) : Mathf.InverseLerp(0f, -90f, wrappedAngle));
		_calc_sideFactorL = Mathf.Lerp(_calc_sideFactorL, num3, _calc_deltaSpeed * 2f);
		if (_wrappedAngle < 0f)
		{
	...
	public override void OnLateUpdatePreApply(LegsAnimator.LegsAnimatorCustomModuleHelper helper)
	{
		...
		float num = _play_HipsRedir.GetFloat();
		float num2 = _play_FeetRedir.GetFloat();
		float num3 = _play_KneesRedir.GetFloat();
		float num4 = _play_AdjustStretch.GetFloat();
		float num5 = Mathf.Abs(_wrappedAngle);
		float num6 = Mathf.InverseLerp(0f, 45f, num5);
		float wrappedAngle = _wrappedAngle;
		float num7 = wrappedAngle;
		if (num2 > 0f)
		{
			if (wrappedAngle < -90f)
			{
				num7 = ((!(wrappedAngle > -135f)) ? Mathf.Lerp(40f, 0f, Mathf.InverseLerp(-135f, -180f, wrappedAngle)) : Mathf.Lerp(-90f, 40f, Mathf.InverseLerp(-90f, -135f, wrappedAngle)));
			}
		...
		for (int i = 0; i < base.LA.Legs.Count; i++)
		{
			...
			if (num3 > 0f)
			{
				if (leg.Side == LegsAnimator.ELegSide.Left)
				{
					leg.IKProcessor.StartBoneRotationOffset = Quaternion.Euler(0f, (0f - _calc_sideFactorR) * Mathf.Min(35f, 50f * num3) * num10, 0f);
				}
				else if (leg.Side == LegsAnimator.ELegSide.Right)
				{
					leg.IKProcessor.StartBoneRotationOffset = Quaternion.Euler(0f, (0f - _calc_sideFactorL) * Mathf.Min(35f, 50f * num3) * num10, 0f);
				}
			}
			else
			{
			...
			{
				float num13 = Mathf.InverseLerp(0.9f, 1.125f, stretchValue);
				if (leg.Side == LegsAnimator.ELegSide.Left)
				{
					num8 += num13;
				}
		...
		if (num5 > 0f)
		{
			if (num5 < 90f)
			{
				float t = InverseLerpDoubleSide(0f, 45f, num5, 90f);
			...
			if (num5 > 45f && num5 < 135f)
			{
				float t2 = InverseLerpDoubleSide(45f, 90f, num5, 135f);
			...
			if (num5 > 90f)
			{
				float t3 = InverseLerpDoubleSide(90f, 135f, num5, 180f);
			...
			if (num5 > 135f)
			{
				float t4 = Mathf.InverseLerp(135f, 180f, num5);
		...
		_calc_rStretch = Mathf.Lerp(_calc_rStretch, num9, _calc_deltaSpeed);
		val2.y -= _calc_lStretch * User_StretchRotatorAnglePower * num * num14;
		val2.y += _calc_rStretch * User_StretchRotatorAnglePower * num * num14;
		...
		float num15 = _play_RestoreSpine.GetFloat();
		if ((Object)(object)SpineBone != (Object)null)
		{
