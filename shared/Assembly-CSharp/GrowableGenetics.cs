...
public static class GrowableGenetics
{
	...
	public static void CrossBreed(GrowableEntity growable)
	{
		...
		if ((Object)(object)growable.GetPlanter() == (Object)null)
		{
			return;
		}
		...
		try
		{
			...
			{
				float num = Vector3.Distance(((Component)child).transform.position, ((Component)growable).transform.position);
				if (child is GrowableEntity item && num <= 1.5f)
				{
					((List<GrowableEntity>)(object)val).Add(item);
				}
			}
			bool flag = false;
			for (int i = 0; i < 6; i++)
			{
				...
				GeneWeighting dominantGeneWeighting = GetDominantGeneWeighting(growable, (List<GrowableEntity>)(object)val, i);
