...
public class MarchingCubesGenerator : IDisposable
{
	...
	public JobHandle ScheduleMarch()
	{
		...
		Facepunch.MarchingCubes.MarchJob marchJob = new Facepunch.MarchingCubes.MarchJob
		{
			sampler = _sampler,
			vertices = vertices0,
			indices = indices0,
		...
		Facepunch.MarchingCubes.CleanupDuplicateVerticesJob obj = new Facepunch.MarchingCubes.CleanupDuplicateVerticesJob
		{
			inputVertices = vertices0,
			inputIndices = indices0,
			outputVertices = vertices1,
			outputIndices = indices1,
			indexToIndices = indexToIndices,
			vertexOffset = vertexOffset,
			invScale = math.rcp(_scale),
			width = _sampler.Width,
			widthHeight = _sampler.Height * _sampler.Width
		};
		JobHandle val = IJobExtensions.Schedule<Facepunch.MarchingCubes.MarchJob>(marchJob, default(JobHandle));
		return IJobExtensions.Schedule<Facepunch.MarchingCubes.CleanupDuplicateVerticesJob>(obj, val);
	}
	...
	public unsafe void ApplyUpdate(bool fromSimplify = false)
	{
