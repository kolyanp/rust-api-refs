...
public class HarborCraneContainerPickup : HarborCrane
{
	...
	private QueuedMove CreateMove(QueuedMove from)
	{
		...
		{
			TargetEntity = new EntityRef<CargoShipContainer>(from.targetEntity),
			TargetWorldPosition = from.targetWorldPosition,
			TargetWorldRotation = new Quaternion(from.targetWorldRotation.x, from.targetWorldRotation.y, from.targetWorldRotation.z, from.targetWorldRotation.w),
			HasTarget = from.hasTarget
		};
	}
