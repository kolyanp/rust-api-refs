...
public class CompleteTrain : IDisposable
{
	...
	public void UpdateTick(float dt)
	{
		...
		if (!disposed)
		{
			...
			if (!HasAnyEnginesOn() && !HasAnyCollisions() && !IsCinematicActive && Time.time > lastMovingTime + 10f)
			{
				trackSpeed = 0f;
				SleepAll();
			}
	...
	private void MovementTick(float dt)
	{
		...
		if (isCinematicBraking)
		{
			float num = cinematicBrakeForce / TotalMass * dt;
			trackSpeed = Mathf.MoveTowards(trackSpeed, 0f, num);
			if (trackSpeed == 0f)
			{
		...
		{
			float num2 = cinematicAccelForce / TotalMass * dt;
			trackSpeed = Mathf.MoveTowards(trackSpeed, cinematicTargetSpeed, num2);
		...
		else
		{
			...
			else
			{
				...
				float num4 = shuntDistance - Vector3.Distance(Vector2.op_Implicit(shuntStartPos2D), Vector2.op_Implicit(shuntTargetPos2D));
				if (num4 < 2f)
				{
					float num5 = Mathf.InverseLerp(0f, 2f, num4);
					num3 *= Mathf.Lerp(0.1f, 1f, num5);
				}
				trackSpeed = Mathf.MoveTowards(trackSpeed, flag ? num3 : (0f - num3), dt * 10f);
				if (TimeSince.op_Implicit(timeSinceShuntStart) > 20f || num4 <= 0f)
				{
		...
		{
			float num6 = trainCars[0].rigidBody.linearDamping;
			if (IsLinedUpToUnload)
			{
				float num7 = Mathf.Abs(trackSpeed);
				if (num7 > 1f)
				{
					...
					{
						float num8 = trainCarUnloadable.MinDistToUnloadingArea(unloaderPos);
						float num9 = Mathf.InverseLerp(2f, 0f, num8);
						if (num7 < 2f)
						{
							float num10 = (num7 - 1f) / 1f;
							num9 *= num10;
						}
						num6 = Mathf.Lerp(num6, 3.5f, num9);
					}
				}
			}
			if (trackSpeed > 0f)
			{
				trackSpeed -= num6 * 4f * dt;
				if (trackSpeed < 0f)
				{
					trackSpeed = 0f;
				}
		...
		for (int num12 = trainCars.Count - 2; num12 >= 0; num12--)
		{
			MoveOtherTrainCar(trainCars[num12], trainCars[num12 + 1]);
		}
	}
	...
	private float ApplyCollisions(float trackSpeed, TrainCar ourTrainCar, Vector3 forwardVector, bool atOurFront, TriggerTrainCollisions trigger, float ourTotalMass, ref StaticCollisionState wasStaticColliding, StaticCollisionState otherEndStaticColliding, float deltaTime)
	{
		...
		if (!flag && otherEndStaticColliding == StaticCollisionState.Free)
		{
			...
			foreach (Rigidbody otherRigidbodyContent in trigger.otherRigidbodyContents)
			{
				...
				num += Vector3.Magnitude(otherRigidbodyContent.linearVelocity - val) * Mathf.Clamp(otherRigidbodyContent.mass, 1f, 13000f);
			}
		}
	...
	private float GetTotalPushingMass(Vector3 pushDirection, Vector3 ourForward, ref List<CompleteTrain> prevTrains)
	{
		...
		if ((flag ? staticCollidingAtFront : staticCollidingAtRear) != StaticCollisionState.Free)
		{
			return -1f;
		}
