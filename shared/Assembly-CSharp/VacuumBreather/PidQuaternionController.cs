...
public class PidQuaternionController
{
	...
	public Vector3 ComputeRequiredAngularAcceleration(Quaternion currentOrientation, Quaternion desiredOrientation, Vector3 currentAngularVelocity, float deltaTime)
	{
		...
		Quaternion error = QuaternionExtensions.Subtract(Quaternion.identity, val);
		Quaternion delta = ToEulerAngleQuaternion(currentAngularVelocity) * val;
		Matrix4x4 matrix = new Matrix4x4
		{
			m00 = (0f - val.x) * (0f - val.x) + (0f - val.y) * (0f - val.y) + (0f - val.z) * (0f - val.z),
			m01 = (0f - val.x) * val.w + (0f - val.y) * (0f - val.z) + (0f - val.z) * val.y,
			m02 = (0f - val.x) * val.z + (0f - val.y) * val.w + (0f - val.z) * (0f - val.x),
			m03 = (0f - val.x) * (0f - val.y) + (0f - val.y) * val.x + (0f - val.z) * val.w,
			m10 = val.w * (0f - val.x) + (0f - val.z) * (0f - val.y) + val.y * (0f - val.z),
			m11 = val.w * val.w + (0f - val.z) * (0f - val.z) + val.y * val.y,
			m12 = val.w * val.z + (0f - val.z) * val.w + val.y * (0f - val.x),
			m13 = val.w * (0f - val.y) + (0f - val.z) * val.x + val.y * val.w,
			m20 = val.z * (0f - val.x) + val.w * (0f - val.y) + (0f - val.x) * (0f - val.z),
			m21 = val.z * val.w + val.w * (0f - val.z) + (0f - val.x) * val.y,
			m22 = val.z * val.z + val.w * val.w + (0f - val.x) * (0f - val.x),
			m23 = val.z * (0f - val.y) + val.w * val.x + (0f - val.x) * val.w,
			m30 = (0f - val.y) * (0f - val.x) + val.x * (0f - val.y) + val.w * (0f - val.z),
			m31 = (0f - val.y) * val.w + val.x * (0f - val.z) + val.w * val.y,
			m32 = (0f - val.y) * val.z + val.x * val.w + val.w * (0f - val.x),
			m33 = (0f - val.y) * (0f - val.y) + val.x * val.x + val.w * val.w
		};
		Quaternion quaternion = ComputeOutput(error, delta, deltaTime);
		quaternion = MultiplyAsVector(matrix, quaternion);
		Quaternion val2 = QuaternionExtensions.Multiply(quaternion, -2f) * Quaternion.Inverse(val);
		return new Vector3(val2.x, val2.y, val2.z);
	}
