...
public class WaypointRace : BaseEntity
{
	...
	public static void startRace(ConsoleSystem.Arg arg)
	{
		...
		WaypointRaceTarget.GetWaypoints(text, list);
		if (list.Count == 0)
		{
			arg.ReplyWith("Couldn't find any waypoints for " + text + ", is the name correct?");
			return;
		}
		Debug.Log((object)$"Starting race {text} with {list.Count} waypoints");
