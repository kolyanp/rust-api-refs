...
public class State_Circle : FSMStateBase
{
	...
	public override EFSMStateStatus OnStateUpdate(float deltaTime)
	{
		...
		Vector3 targetPositionNS = origin + new Vector3(Mathf.Sin(num2), 0f, Mathf.Cos(num2)) * num;
		targetPositionNS.y = Mathf.Lerp(origin.y, ((Component)Owner).transform.position.y, Mathf.InverseLerp(0f, Vector3.Distance(origin, ((Component)Owner).transform.position), num));
		if (base.Agent.Raycast(targetPositionNS, out var _))
		{
			return EFSMStateStatus.Failure;
		}
		if (!base.Agent.SetDestinationWithParams(targetPositionNS, autoBraking: false, speed))
		{
			return EFSMStateStatus.Failure;
		}
