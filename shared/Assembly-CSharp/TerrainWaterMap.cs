...
public class TerrainWaterMap : TerrainMap<short>
{
	...
	public void GetHeights(NativeArray<Vector3> worldPos, NativeArray<float> results)
	{
		...
		GetHeightByPosJob getHeightByPosJob = new GetHeightByPosJob
		{
			Heights = results,
			Pos = worldPos,
			Data = src,
		...
		IJobExtensions.RunByRef<GetHeightByPosJob>(ref getHeightByPosJob);
	}
	...
	public WaterMapQueryStructure GetQueryStructure()
	{
		...
		return new WaterMapQueryStructure
		{
			res = res,
			Position = Vector2.op_Implicit(TerrainMeta.Position),
			Size = Vector2.op_Implicit(TerrainMeta.Size),
			src = src.AsReadOnly()
		};
	}
	...
	public void GetHeights(NativeArray<Vector2> uvs, NativeArray<float> results)
	{
		...
		//IL_0034: Unknown result type (might be due to invalid IL or missing references)
		//IL_0045: Unknown result type (might be due to invalid IL or missing references)
		GetHeightByUVJob getHeightByUVJob = new GetHeightByUVJob
		{
			Heights = results,
			UV = uvs,
			Data = src,
	...
	public void GetHeightsIndirect(ReadOnly<Vector2> uvs, ReadOnly<int> indices, NativeArray<float> results)
	{
		...
		GetHeightByUVJobIndirect getHeightByUVJobIndirect = new GetHeightByUVJobIndirect
		{
			Heights = results,
			UV = uvs,
			Indices = indices,
		...
		IJobExtensions.RunByRef<GetHeightByUVJobIndirect>(ref getHeightByUVJobIndirect);
	}
	...
	public void GetHeights(NativeArray<Vector2i> indices, NativeArray<float> results)
	{
		...
		//IL_0034: Unknown result type (might be due to invalid IL or missing references)
		//IL_0045: Unknown result type (might be due to invalid IL or missing references)
		GetHeightByIndexJob getHeightByIndexJob = new GetHeightByIndexJob
		{
			Heights = results,
			Indices = indices,
			Data = src,
		...
		IJobExtensions.RunByRef<GetHeightByIndexJob>(ref getHeightByIndexJob);
	}
