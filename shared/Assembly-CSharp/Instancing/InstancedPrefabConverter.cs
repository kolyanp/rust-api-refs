	...
	public static InstancedPrefabConfig ExtractInstancedRenderers(GameObject prefab, uint prefabId, IEnumerable<InstancedMeshFilter> instancedFilters)
	{
		...
		foreach (InstancedMeshFilter instancedFilter in instancedFilters)
		{
			...
			else if ((Object)(object)instancedFilter.MeshLOD != (Object)null)
			{
				...
				{
					MeshLOD.State state = array[num2];
					...
					if (!((Object)(object)component == (Object)null))
					{
						...
						MeshLOD.State? state2 = array.ElementAtOrDefault(num2 + 1);
						float minDistance = ((num2 == 0) ? 0f : state.distance);
						float maxDistance = state2?.distance ?? num;
						instancedLODState = new InstancedLODState(localToWorldMatrix, component, minDistance, maxDistance, num2, array.Length, meshCategory);
						instancedMeshConfig.states.Add(instancedLODState);
					}
				}
			}
			else
			{
				...
				{
					RendererLOD.State state3 = array2[num3];
					...
					if (!((Object)(object)val == (Object)null))
					{
						...
						RendererLOD.State? state4 = array2.ElementAtOrDefault(num3 + 1);
						float minDistance2 = ((num3 == 0) ? 0f : state3.distance);
						float maxDistance2 = state4?.distance ?? num;
						instancedLODState = new InstancedLODState(localToWorldMatrix2, val, minDistance2, maxDistance2, num3, array2.Length, meshCategory);
						instancedMeshConfig.states.Add(instancedLODState);
					}
				}
			}
