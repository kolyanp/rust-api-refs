...
public class RenderManager
{
	...
	private InstancedMeshRenderer RegisterRenderer(InstancedLODState lod)
	{
		...
		for (int num = 0; num < instancedMeshRenderer.DrawCallCount; num++)
		{
			Mesh mesh = instancedMeshRenderer.Mesh;
			drawCallManager.AddDrawCall(instancedMeshRenderer, num, mesh.GetIndexCount(num), mesh.GetIndexStart(num), mesh.GetBaseVertex(num), instancedMeshRenderer.MultidrawSubmeshes[num]);
		}
		rendererLookup[key] = instancedMeshRenderer;
		Renderers.Add(instancedMeshRenderer);
		InstancedRendererJobData instancedRendererJobData = new InstancedRendererJobData
		{
			Id = count,
			MinDistance = lod.MinimumDistance,
			MaxDistance = lod.MaximumDistance,
			ShadowMode = instancedMeshRenderer.CastShadows,
			DrawCallCount = instancedMeshRenderer.DrawCallCount
		};
		EnsureJobDataCapacity();
		RendererArray[count] = instancedRendererJobData;
