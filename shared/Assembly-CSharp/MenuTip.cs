...
public class MenuTip : MonoBehaviour
{
	...
	public void SetCustomTip(string iconString, string tip)
	{
		...
		if (!Enum.TryParse<Icons>(iconString, true, out Icons result))
		{
			result = (Icons)61737;
		}
		icon.SetIcon(result);
		text.SetText(StringExtensions.Truncate(StringEx.FilterRichText(tip, true, new string[5] { "color", "br", "u", "b", "i" }), 400, (string)null));
		disabled = true;
	}
