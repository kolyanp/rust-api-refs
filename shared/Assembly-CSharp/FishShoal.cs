...
public class FishShoal : IDisposable
{
	...
	private unsafe void UpdateJobs(float3 cameraPosition)
	{
		...
		if (num != 0)
		{
			...
			FishCollisionGatherJob fishCollisionGatherJob = new FishCollisionGatherJob
			{
				layerMask = -1,
				seed = seed,
				castCount = castCount,
				...
				fishCastIndices = fishCastIndices,
				fishDataArray = fishData,
				fishRenderDataArray = fishRenderData
			};
			FishCollisionProcessJob fishCollisionProcessJob = new FishCollisionProcessJob
			{
				castCount = castCount,
			...
			FishUpdateJob fishUpdateJob = new FishUpdateJob
			{
				cameraPosition = cameraPosition,
				seed = seed,
				dt = Time.deltaTime,
				minSpeed = fishType.minSpeed,
				maxSpeed = fishType.maxSpeed,
				minTurnSpeed = fishType.minTurnSpeed,
			...
			jobHandle = IJobExtensions.Schedule<FishCollisionGatherJob>(fishCollisionGatherJob, default(JobHandle));
			jobHandle = RaycastCommand.ScheduleBatch(castCommands, castResults, 5, jobHandle);
			jobHandle = IJobExtensions.Schedule<FishCollisionProcessJob>(fishCollisionProcessJob, jobHandle);
			jobHandle = IJobParallelForExtensions.Schedule<FishUpdateJob>(fishUpdateJob, num, 10, jobHandle);
			jobHandle = IJobExtensions.Schedule<KillFish>(killFish, jobHandle);
		}
	}
