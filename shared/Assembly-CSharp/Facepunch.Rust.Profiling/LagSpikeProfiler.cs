...
public static class LagSpikeProfiler
{
	...
	public static void RPC(TimeSpan time, Message packet, BaseEntity entity, uint rpcId)
	{
		if (enabled)
		{
			...
			AddPendingRecord(CreateRecord(time, "rpc").AddField("entity", (BaseNetworkable)entity).AddField("rpc", value).AddField("connection_user", (packet?.connection?.userid).GetValueOrDefault()));
		}
	}
