...
public static class ItemSearchUtils
{
	...
	public static bool IsValidSearchResult(string search, ItemDefinition target, bool checkItemIsValid)
	{
		...
		if (!StringEx.Contains(target.shortname, search, CompareOptions.IgnoreCase) && !StringEx.Contains(target.displayName.translated, search, CompareOptions.IgnoreCase) && (!flag || !StringEx.Contains(translated, search, CompareOptions.IgnoreCase)) && CultureInfo.CurrentCulture.CompareInfo.IndexOf(target.displayName.translated, search, CompareOptions.IgnoreCase | CompareOptions.IgnoreNonSpace) < 0)
		{
			if (flag)
			{
	...
	public static float ScoreSearchResult(string search, ItemDefinition target)
	{
		...
		num -= 50f * num4;
		if (!string.IsNullOrEmpty(target.displayDescription.translated) && StringEx.Contains(target.displayDescription.translated, search, CompareOptions.IgnoreCase))
		{
