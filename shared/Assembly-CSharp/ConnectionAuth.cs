...
public class ConnectionAuth : MonoBehaviour
{
	...
	{
		DebugEx.Log(connection.ToString() + " Rejecting connection - " + (string.IsNullOrEmpty(strReasonPrivate) ? strReason : strReasonPrivate), (StackTraceLogType)0);
	...
	public void OnNewConnection(Connection connection)
	{
		...
		if (connection.protocol != 2632)
		{
			...
			DebugEx.Log("Not kicking " + connection.userid + " for incompatible protocol (is a developer)", (StackTraceLogType)0);
		}
		...
		{
			DebugEx.Log(connection.ToString() + " has auth level 1", (StackTraceLogType)0);
			connection.authLevel = 1u;
		}
		if (ServerUsers.Is(connection.userid, ServerUsers.UserGroup.Owner))
		{
			DebugEx.Log(connection.ToString() + " has auth level 2", (StackTraceLogType)0);
			connection.authLevel = 2u;
		}
		if (DeveloperList.Contains(connection.userid))
		{
			DebugEx.Log(connection.ToString() + " is a developer", (StackTraceLogType)0);
