...
public class BlackjackController(BaseCardGameEntity owner) : CardGameController(owner)
{
	[Flags]
	...
	public void InputsToList(int availableInputs, List<BlackjackInputOption> result)
	{
		...
		{
			if (blackjackInputOption != BlackjackInputOption.None && ((uint)availableInputs & (uint)blackjackInputOption) == (uint)blackjackInputOption)
			{
	...
	protected override void SubEndRound()
	{
		...
		BlackjackRoundResult CheckResult(List<PlayingCard> cards, int betAmount, out int reference)
		{
			...
			case BlackjackRoundResult.BlackjackWin:
				reference = Mathf.FloorToInt((float)betAmount * 2.5f);
				break;
			case BlackjackRoundResult.Win:
				reference = Mathf.FloorToInt((float)betAmount * 2f);
				break;
