...
public class ZiplineLaunchPoint : BaseEntity
{
	...
	public void FindZiplineTarget(ref List<Vector3> foundPositions)
	{
		...
		Pool.FreeUnmanaged<ZiplineTarget>(ref list);
	}
	...
	public static void report(ConsoleSystem.Arg arg)
	{
		...
		arg.ReplyWith(string.Format("{0} ziplines, total distance: {1:F2}, avg length: {2:F2}, arrival points: {3}", new object[4]
		{
			num2,
			num,
			num / (float)num2,
	...
	public void UpdateBuildingBlocks()
	{
		...
		for (int j = 0; j < linePoints.Count; j++)
		{
			...
			if (val != Vector3.zero && Vector3.Dot(normalized, val) < 0.98f)
			{
				...
				{
					SetUpBuildingBlock(BuildingBlocks[num], PointBuildingBlocks[num], SpawnableBoundsBlockers[num++], startIndex, j - 1);
				}
				startIndex = j - 1;
			}
			val = normalized;
		...
		if (num < BuildingBlocks.Length)
		{
			SetUpBuildingBlock(BuildingBlocks[num], PointBuildingBlocks[num], SpawnableBoundsBlockers[num], startIndex, linePoints.Count - 1);
		}
		void SetUpBuildingBlock(BoxCollider longCollider, BoxCollider pointCollider, SpawnableBoundsBlocker spawnBlocker, int num2, int endIndex)
		{
			//IL_0008: Unknown result type (might be due to invalid IL or missing references)
			//IL_000d: Unknown result type (might be due to invalid IL or missing references)
