...
public static class BuoyancyForceAccumulationBurst
{
	...
	internal static class Compute_000058F5_0024BurstDirectCall
	{
		...
		private static void GetFunctionPointerDiscard(ref IntPtr P_0)
		{
			...
			if (Pointer == (IntPtr)0)
			{
				Pointer = BurstCompiler.CompileFunctionPointer<Compute_000058F5_0024PostfixBurstDelegate>((Compute_000058F5_0024PostfixBurstDelegate)Compute).Value;
			}
			P_0 = Pointer;
		}
	...
	[BurstCompile]
	[MonoPInvokeCallback(typeof(Compute_000058F5_0024PostfixBurstDelegate))]
	public static void Compute(in NativeArray<InstanceInput> instances, in NativeArray<float3> allPositions3D, in NativeArray<float> pointShoreDistance, in NativeArray<WaterLevel.WaterInfo> pointWaterInfo, in NativeArray<float> pointSize, in NativeArray<float> pointBuoyancyForce, in NativeArray<float> pointRandomOffset, in NativeArray<float> pointWaveFrequency, in NativeArray<float> pointWaveScale, in NativeArray<float3> pointFlowDirection, float time, ref NativeArray<InstanceOutput> results)
	{
		Compute_000058F5_0024BurstDirectCall.Invoke(in instances, in allPositions3D, in pointShoreDistance, in pointWaterInfo, in pointSize, in pointBuoyancyForce, in pointRandomOffset, in pointWaveFrequency, in pointWaveScale, in pointFlowDirection, time, ref results);
	}
