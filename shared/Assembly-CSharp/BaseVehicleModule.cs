...
public class BaseVehicleModule : BaseVehicle, IPrefabPreProcess
{
	...
	public virtual Phrase CannotBeMovedNowReason()
	{
		return null;
	}
		...
		if (flag)
		{
			...
			else
			{
				for (int j = 0; j < conditional.socketSettings.Length; j++)
				{
					...
					if (socket == null)
					{
						Debug.LogWarning((object)string.Format("{0} module got NULL socket at index {1}. Total vehicle sockets: {2} FirstSocketIndex: {3} Sockets taken: {4}", new object[5]
						{
							AssociatedItemDef.displayName.translated,
							FirstSocketIndex + j,
							Vehicle.TotalSockets,
							FirstSocketIndex,
