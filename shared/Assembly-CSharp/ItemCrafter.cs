...
public class ItemCrafter : EntityComponent<BasePlayer>
{
	...
	public void ServerUpdate(float delta)
	{
		...
		if (value.endTime == 0f)
		{
			...
			if ((Object)(object)value.workbenchEntity != (Object)null && value.workbenchEntity is Workbench workbench)
			{
				num *= workbench.GetCraftSpeedMultiplier(value);
			}
			value.endTime = Time.realtimeSinceStartup + num;
	...
	public bool CraftItem(ItemBlueprint bp, BasePlayer owner, InstanceData instanceData = null, int amount = 1, int skinID = 0, Item fromTempBlueprint = null, bool free = false, int attachmentID = 0)
	{
	...
	public void FinishCrafting(ItemCraftTask task)
	{
		...
		Item item = ItemManager.CreateByItemID(task.blueprint.targetItem.itemid, 1, skin, (ulong)task.attachmentID);
		...
		if ((Object)(object)task.workbenchEntity != (Object)null && task.workbenchEntity is Workbench workbench)
		{
			workbench.ApplyUpgradesToCraftedItem(owner, task, item);
			workbench.GiveBonusItems(owner, task, item);
		}
