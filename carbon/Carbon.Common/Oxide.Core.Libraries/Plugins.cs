...
public class Plugins : Library
{
	...
	{
		return Community.Runtime.Plugins.Plugins.Any((RustPlugin x) => x.Name == name);
	}
	...
	public Plugin Find(string name)
	{
		...
		foreach (ModLoader.Package package in ModLoader.Packages)
		{
			...
			{
				if (plugin.Name.Replace(" ", "").Replace(".", "") == name)
				{
					return plugin;
				}
			}
	...
	public Plugin[] GetAll()
	{
		...
		foreach (ModLoader.Package package in ModLoader.Packages)
		{
			list.AddRange(package.Plugins);
		}
		Plugin[] result = list.ToArray();
	...
	public void GetAllNonAlloc(List<RustPlugin> buffer)
	{
		foreach (ModLoader.Package package in ModLoader.Packages)
		{
			buffer.AddRange(package.Plugins);
		}
	}
