...
{
	internal static string[] _dotSplit = new string[1] { "." };
	...
	{
		Logger.Log(string.Format("CallStack: {0}{1}", new StackTrace(1, fNeedFileInfo: true)));
	}
	...
	{
		string[] array = ipAddress.Split(_dotSplit, StringSplitOptions.RemoveEmptyEntries);
	...
	public static bool ValidateIPv4(string ipAddress)
	{
		...
		string[] array = ipAddress.Replace("\"", string.Empty).Trim().Split(_dotSplit, StringSplitOptions.RemoveEmptyEntries);
		if (array.Length == 4)
		{
