...
public class Player : Library
{
	...
	public CultureInfo Language(BasePlayer player)
	{
		try
		{
			return CultureInfo.GetCultureInfo(((BaseNetworkable)player).net.connection.language ?? "en");
		}
	...
	public void Ban(ulong id, string reason = "", long expiry = -1L)
	{
		if (!IsBanned(id))
		{
			...
			ServerUsers.Set(id, (UserGroup)3, (((Object)(object)val != (Object)null) ? val.displayName : null) ?? "Unknown", reason, expiry);
			ServerUsers.Save();
	...
	{
		player.inventory.GiveItem(ItemManager.CreateByItemID(item.info.itemid, quantity, 0uL, 0uL), (ItemContainer)null, (GiveItemOptions)0);
	}
